...
module.exports = class InputValidator {
  ...
  //database validation constraints should handle validation errors?
  //or should we do it here to make it more clear what went wrong 
  static validateGoalUpdate(updateInfo) {
    ...
    Object.keys(updateInfo).forEach(function (key) {
      if (key === "name" && !updateInfo.key) {
      ...
      else if (key === "intensity" && !updateInfo.key) {
        ...
        validationErrorMessages.push("update field not supported by goal: " + key);
      }
    })
    if (validationErrorMessages.length > 0) {
      throw new ValidationError('The goal info you submitted is not valid: ', validationErrorMessages);
  ...
  static validationEntryCreation(entry_info) {
    let validationErrorMessages = [];
    if (!entry_info.date) {
      validationErrorMessages.push("Health log entries needs date")
    }
    if (!entry_info.weight) {
      validationErrorMessages.push("Health log entries needs weight")
    }
    if (validationErrorMessages.length > 0) {
      throw new ValidationError('The goal info you submitted is not valid: ', validationErrorMessages);
    }
  }
  static validationEntryUpdate(entry_info) {
